# Claude Code Project Instructions — landlock-kotlin

## Project Overview

This is **landlock-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`landlock`](https://crates.io/crates/landlock).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/landlock/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/landlock>
