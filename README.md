...
Kotlin Multiplatform line-by-line clean-room port of the Rust crate [`landlock`](https://crates.io/crates/landlock).

> **Status: scaffold — porting has not started.** This repo currently contains build infrastructure only. The upstream Rust source for the `landlock` crate will be cloned into `tmp/` (gitignored) when porting begins.

## About

Linux Landlock LSM bindings
...
Original work copyrighted by the upstream `landlock` authors. Kotlin port copyright (c) 2026 Sydney Renee and The Solace Project.
